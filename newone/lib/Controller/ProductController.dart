import 'package:get/get.dart';

import '../Model/ProductsModel.dart';
import 'package:http/http.dart' as http;

class ProductController extends GetxController{

  RxList<Product> rxProducts = <Product>[].obs;

  var noImage ="data:image/jpeg;base64,".obs;





  getAllProduct() async{


    try{

      final response = await http.get(
        Uri.parse("https://dummyjson.com/products"),

      );

      if (response.statusCode == 200) {
        final responseBody = response.body;
        print("Date " + responseBody);
        rxProducts.assignAll(productsModelFromJson(response.body).products);


      }


    }catch(e){


    }


  }

  @override
  void onInit() {
    getAllProduct();
    super.onInit();
  }


  select(int index)
  {

    if(rxProducts.toList()[index].selected==true)
      {
        rxProducts.toList()[index].selected=false;

        print(" Flag 1");
      }
    else
      {
        rxProducts.toList()[index].selected=true;
        print(" Flag 1");
      }

  }



}